...
int main()
{
    ...
    printf("Enter the size of the matrix:"); //enter the matrix's size : 3
    scanf("%d",&n);
    int a[n][n]; //declare an array with the size
    ...
    /*      this loop is used for the summation of the right diagonals*/
    for(i=0;i<n;i++) //Used for no of rows in a matrix
    {
        sum+=a[i][n-i-1]; //0+3...3+5......8+7
    }
    printf("Sum of right diagonal: %d",sum);// 15
    return 0;
}